...
shared_ptr<Program> prog, tex_prog, tex2;
...
static void initGL()
{
  ...
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthBuf);
	...
	tex_prog->init();
	tex_prog->addUniform("frame");
	tex_prog->addUniform("texBuf");
	tex_prog->addAttribute("vertPos");

	tex2 = make_shared<Program>();
	tex2->setVerbose(true);
	tex2->setShaderNames(RESOURCE_DIR + "pass_vert.glsl", RESOURCE_DIR + "tex_frag_horiz.glsl");
	tex2->init();
	tex2->addUniform("frame");
	tex2->addUniform("texBuf");
	tex2->addAttribute("vertPos");

}

/* To complete image processing on the specificed texture */
/* Right now this just draws large quad to the screen that is texture mapped
   with the prior scene image */
/* TODO: fill in with call to appropriate shader(s) to complete lab tasks */
void ProcessImage(GLuint inTex, int idx) {
   glActiveTexture(GL_TEXTURE0);
   glBindTexture(GL_TEXTURE_2D, inTex);
   
   switch (idx) {
      case 0:
         // example applying of 'drawing' the FBO texture - change these shaders
         tex_prog->bind();
         glUniform2f(tex_prog->getUniform("frame"), (float)g_width, (float)g_height);
      ...
      case 1:
         tex2->bind();
         glUniform2f(tex2->getUniform("frame"), (float)g_width, (float)g_height);
         glUniform1i(tex2->getUniform("texBuf"), 0);
         glEnableVertexAttribArray(0);
         glBindBuffer(GL_ARRAY_BUFFER, quad_vertexbuffer);
         glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
         glDrawArrays(GL_TRIANGLES, 0, 6);
...
static void render()
{
   ...
   for (int i = 0; i < 59; i++) {
      if ((i%2) == 0) {
   	   /* set up the correct framebuffer and shaders to use depending on task*/
	      glBindFramebuffer(GL_FRAMEBUFFER, frameBuf[1]);
	      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	      /* draw the actual output  to the framebuffer*/	
	      ProcessImage(texBuf[0], 0);
      }
      else {
	      glBindFramebuffer(GL_FRAMEBUFFER, frameBuf[0]);
	      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	      ProcessImage(texBuf[1], 1);
      }
   }
   ...
   ProcessImage(texBuf[1], 1);
	...
	/* code to write out the FBO (texture) just once provided as an example*/
	if (FirstTime) {
