...
DP_Ops decode (const DP_Type data) {
  if (data.instr.DP_Instr.op == DP_RSB) {
    if (opts.instrs) {
      cout << "neg r" << data.instr.DP_Instr.rdn << ", r" << data.instr.DP_Instr.rm << endl;
    }
    return DP_RSB;
  }
  if (data.instr.DP_Instr.op == DP_CMP_OP) {
    if (opts.instrs) {
      cout << "cmp ";
      if (data.instr.DP_Instr.rdn == 12) 
         cout << "ip";
      else 
         cout << "r" << data.instr.DP_Instr.rdn;

      cout << ", r" << setbase(10) << (data.instr.DP_Instr.rm) << endl;
    }
    return DP_CMP;
  }
}
...
SP_Ops decode (const SP_Type data) {
  if (data.instr.mov.op == 2) {
    if (opts.instrs) { 
      ...
      if (data.instr.mov.d) {
        ...
        /*else if (data.instr.mov.rm == 12) {
          cout << " r" << setbase(10) << (8+data.instr.mov.rd) << ", ip" << endl;
        }*/
        else {
      ...
      /*else if (data.instr.mov.rm == 12) {
        cout << " r" << data.instr.mov.rd << ", ip" << endl;
      }*/
      else {
  ...
  else if (data.instr.add.op == SP_ADD_OP) {
    if (opts.instrs) { 
      cout << "add";
      if (data.instr.add.d) {
        ...
        /*else if (data.instr.add.rd == 4) {
          cout << " ip, ip, r" << setbase(10) << data.instr.add.rm << endl;
        }
        else if (data.instr.add.rm == 12) {
          cout << " r" << setbase(10) << (8+data.instr.add.rd) << ", r" << setbase(10) << (8+data.instr.add.rd) << ", ip" << endl;
        }*/
	else {
           cout << " r" << setbase(10) << data.instr.add.rd+8 << ", r" << setbase(10) << data.instr.add.rd+8 << ", r" << setbase(10) << (data.instr.add.rm) << endl;
...
int decode (const LDM_Type data) {
  ...
  if (opts.instrs) {
     ...
     total = 0;
     for (int i = 0; i < 8; i++) {
       if (data.instr.ldm.reg_list & bit) {
         if (total == 0)
           cout << "r" << i;
         else
           cout << ", r" << i;
...
int decode (const STM_Type data) {
  ...
  if (opts.instrs) {
     ...
     total = 0;
     for (int i = 0; i < 8; i++) {
       if (data.instr.stm.reg_list & bit) {
         if (total == 0)
           cout << "r" << i;
         else
           cout << ", r" << i;
