...
bool Cache::access(unsigned int address) {
  int tag, byteSelect, index;
  unsigned int tagMask, indexMask, byteMask;

  tag = 32 - log2(size);
  byteSelect = log2(blocksize);
  index = 32 - tag - byteSelect;

  byteMask = (address >> 0) & ((1 << byteSelect) -1);
  indexMask = (address >> byteSelect) & ((1 << index) -1);
  tagMask = (address >> (byteSelect + index)) & ((1 << tag) -1);
  
  if(entries[indexMask] == tagMask) {
     hits++;
     return true;
  } 
  else {
     entries[indexMask] = tagMask;
     misses++;
     return false;
  }
}
